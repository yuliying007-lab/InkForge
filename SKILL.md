...
**主题选择器（优先使用 show_widget 内联方式）：**
当用户**未指定主题**时，使用 `show_widget` 工具输出内联配色选择器。选择器包含全部 108 套配色的色卡网格，按分组展示，支持搜索和筛选。用户点选色卡后可实时预览文章效果（标题、正文、引用块、分隔符、列表、名片），确认后通过 `sendPrompt` 回传主题代号，Claude 继续执行。

色卡数据和预览模板：读取下方「主题色表」生成色卡。预览区使用用户实际文章的前 200 字（如已提供）或通用示例文本。

**模板选择器：**
当用户**未指定模板**时，输出 `references/template-picker.jsx` 作为 Artifact，用户点击选择后 Claude 从返回消息中提取模板代号，继续执行。

**都未指定时的流程：**
当用户说"帮我排版"并附上了文章内容时，Claude 可以根据自动匹配表直接匹配模板和主题，**不强制弹出选择器**——但应在输出末尾注明所用的模板和主题，并提示用户如需更换可告知。

如果用户明确要求选择，先输出模板选择器，等用户选择后再输出主题选择器（show_widget），分两步完成。
...
**B. 氛围配图（多渠道搜索）**
当文章需要场景照片、氛围图时，Claude 根据文章内容提取 2-3 个关键词，主动搜索图片并将结果以 `<img>` 标签嵌入 HTML 中。

**图片来源优先级：**

1. **Unsplash（通用氛围图）** — 使用 WebSearch 搜索 `site:unsplash.com` + 关键词，获取免费商用图片。直接使用 `https://images.unsplash.com/photo-{id}?w=1080&q=80` 格式嵌入，标注摄影师。
2. **官方渠道（行业/主题相关图）** — 当文章涉及特定机构、大学、品牌、政府时，优先搜索该机构官网、新闻稿、官方社交媒体发布的图片素材。例如：
   - 大学类文章 → 搜索该大学官网 Media/Press 页面
   - 企业类文章 → 搜索企业官方 Press Kit 或新闻发布
