...
3. **标题风格区（中下部）：** 展示全部 15 种标题装饰风格（HEAD-01 ~ HEAD-15），每种以实际渲染效果展示（使用已选配色的 PRIMARY/TEXT 等色值渲染真实 HTML 效果）。每个风格卡片显示：风格名称 + 实时渲染的标题样例。根据已选模板的人格风格，在匹配建议表推荐的风格上标注「推荐」。默认选中模板自带的 section_heading 对应风格（如无精确对应则选 HEAD-01）。用户可点选任一风格覆盖默认。

4. **实时预览区（底部）：** 用户点选模板、配色或标题风格时，下方展示使用**已选模板 + 已选配色 + 已选标题风格**的排版预览。预览使用用户实际文章的前 200-300 字，套用所选模板的 HTML 组件结构（article_header、section_heading、body_paragraph、blockquote、数据卡片等），将主题色值替换到实际渲染。section_heading 使用已选标题风格渲染。预览区 max-height 480px，可滚动。每个模板需预设一个渲染函数，展示该模板的典型组件组合。

5. **确认按钮：** 显示「确认使用：P27 政策解读 × T66 法式优雅 × HEAD-06 荧光笔标记 ↗」，点击通过 `sendPrompt('使用模板：P27 政策解读，配色：T66 法式优雅，标题风格：HEAD-06 荧光笔标记')` 回传选择。
...
**禁止行为：**
- 禁止在用户给出文章后直接自动匹配模板、主题和标题风格开始排版
- 禁止跳过选择器
...
章节标题使用独立的标题风格库。读取 `references/headings.md` 获取 15 种标题装饰风格。
